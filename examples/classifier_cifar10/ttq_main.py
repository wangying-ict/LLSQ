...
def main_worker(gpu, args):
    ...
    if args.dali:
        train_loader = DataLoaders_DALI.get_cifar_iter_dali(type='train', image_dir=args.data,
    ...
    scheduler_step = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30, 100, 150])
    scheduler_next = scheduler_step
    if args.cosine:
        scheduler_next = scheduler_cosine
    scheduler_warmup = GradualWarmupScheduler(optimizer, multiplier=1.1, total_epoch=1,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        ttq_namespace.ratio = 1 + epoch / args.epochs
        print('incremental ratio: {}'.format(ttq_namespace.ratio))
        ...
        if args.debug:
            if len(model.state_dict()['features.0.pos']) > 1:
                writer.add_histogram('val/conv1.pos', model.state_dict()['features.0.pos'], epoch)
                writer.add_histogram('val/conv1.neg', model.state_dict()['features.0.neg'], epoch)
            else:
                writer.add_scalar('val/conv1.pos', model.state_dict()['features.0.pos'], epoch)
                writer.add_scalar('val/conv1.neg', model.state_dict()['features.0.neg'], epoch)

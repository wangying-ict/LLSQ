...
__all__ = ['ResNet', 'seq_resnet18', 'seq_resnet34', 'seq_resnet50', 'seq_resnet101',
           'seq_resnet152']
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        self.inplanes = 64
        self.conv1_seq = nn.Sequential(nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                                                 bias=False),
                                       nn.BatchNorm2d(64),
                                       )
        self.relu = nn.ReLU(inplace=True)
...
def seq_resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        if pretrained:
            load_fake_quantized_state_dict(model, model_zoo.load_url(model_urls['resnet18']), 'seq_resnet18_map.json')
    return model
